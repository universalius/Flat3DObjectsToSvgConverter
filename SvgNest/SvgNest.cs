...
public class SvgNest
{
    ...
    private bool _debugNfsCalculation = false;
    ...
    private async Task<List<XmlElement>> LaunchWorkers(List<Node> tree, PolygonWithBounds binPolygon/*, progressCallback, displayCallback*/)
    {
        ...
        KeyValuePair<SvgNestPair, List<DPath>>?[] generatedNfp = null;
        if (_debugNfsCalculation)
        {
            generatedNfp = nfpPairs.AsParallel().Select(GetNodesPairsPathes).ToArray();
        }
        else
        {
            Console.WriteLine();
            long completed = 0;
            generatedNfp = nfpPairs.AsParallel()
                .WithProgressReporting(() =>
                {
                    Interlocked.Increment(ref completed);
                    Console.Write($"\rFinished nfps {completed * 100 / nfpPairs.Count}%            ");
                })
                .Select(GetNodesPairsPathes).ToArray();
        ...
        //var placementsTasks = new List<List<Node>> { new List<Node>(placelist) }.Select(worker.PlacePaths);
        //var processedPlacements = 0;
        //var placements = await WhenAll(placementsTasks, (tasks) =>
        //{
        //    processedPlacements++;
        //    Console.WriteLine($"Processed {processedPlacements} out of {placementsTasks.Count()}");
        //});
    ...
    public async Task<PlacementsFitness[]> WhenAll(IEnumerable<Task<PlacementsFitness>> tasks, Action<IEnumerable<Task<PlacementsFitness>>> progressAction)
    {
        ...
        for (; ; )
        {
            ...
            {
                //var results = new List<PlacementsFitness>();
